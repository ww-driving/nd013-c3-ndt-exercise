...
double Probability(Eigen::MatrixXd X, Eigen::MatrixXd Q, Eigen::MatrixXd S){
	...
	return std::exp((-(X-Q).transpose() * S.inverse() * (X-Q) / 2)(0,0));
}
...
Cell PDF(PointCloudT::Ptr input, int res, pcl::visualization::PCLVisualizer::Ptr& viewer){
	...
	Eigen::MatrixXd IN(2, input->size());
	...
	Eigen::MatrixXd S(2,2);
	Eigen::MatrixXd X_Q(IN);
	X_Q.colwise() -= Q;
	S << X_Q * X_Q.transpose() / input->size();
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived>& g_previous, Eigen:: MatrixBase<Derived>& H_previous){
	...
	Eigen::MatrixXd Q(cell.Q), S(cell.S);
	...
	H << (-q.transpose() * S.inverse() * J).transpose() * (-q.transpose() * S.inverse() * J) - J.transpose() * S.inverse() * J;
	H(2,2) += -q.transpose() * S.inverse() * q2;
	H = -e * H;
...
int main(){
	...
	if(part1){
		...
		for(int iteration = 0; iteration < 100; iteration++){ // TODO: increase the iteration count to get convergence
			...
			PosDef(H, 0, 0.1, 10); // TODO: change the increment and max values to nonzero values
			...
			if(matching){
				...
				for(PointT point:source->points){
					...
					if(cell.cloud->points.size() > 2){
						...
						Eigen::Affine2d T = Eigen::Translation2d(x, y) * Eigen::Rotation2Dd(theta);
						Eigen::Vector2d pt = T * Eigen::Vector2d(point.x, point.y);
						PointT pointTran(pt(0), pt(1), point.z);
